...
> - Поправил `type="hidden"` для скрытого поля `id` в `detailsForm`
 
### ![error](https://cloud.githubusercontent.com/assets/13649199/13672935/ef09ec1e-e6e7-11e5-9f79-d1641c05cbe6.png) Изменения UI
#### Apply 9_0_2_change_ui
> - Поменял цвет и добавил текст в кнопку добавления +
...
> - `reset()` не чистит скрытые (hidden) поля формы. Сделал очистку поле через `form.find(":input").val("")`    
...
- 4: datetimepicker работает корректно в Хроме, если убрать в `<input` JSP  `type="date/time/datetime-local" ..`
...
- 6: Также не дублируйте обработку ошибок в `BindingResult` в ajax контроллерах
