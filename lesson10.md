...
#### Apply 0_1_fix_ui.patch
> - Поправил сообщения локализации
> - Удалил JSESSIONID cookie, [создаваемые JSP (`<%@ page session="false" %>`)](https://stackoverflow.com/a/17196872/548473)
> - Удалил лишние `<div>` (`shadow` и `view-box`) 

#### Apply 0_2_fix_refactoring.patch
> - Сделал интерфейс `HasId` от которого наследуются `BaseTo` и `BaseEntity`
> - Сделал проверка id в `ValidationUtil` на основе `HasId` 
> - Поправил логирование до проверки `id`
> - Сделал в `ProfileRestController` обновление своего профиля через `UserTo` (нельзя изменять себе роли)
...
#### Apply 05_validated_groups.patch
...
#### Apply 06_secure_tag_annotation.patch
...
#### Apply 09_registration.patch

> - Добавил локализацию
> - При регистрации передаю `username` в `login.jsp` как параметр и делаю `setCredentials` (пароль вводится скрыто, поэтому его не передаю). Т.к `setCredentials` требует jQuery, убрал для него `defer`      
...
#### Apply 11_global_exception.patch
...
#### Apply 12_controller_advice_exception.patch
...
> - Добавил `ValidationUtil.getRootCause` и использую его в `ExceptionInfoHandler.logAndGetErrorInfo()`
...
#### Apply 13_password_encoding.patch
...
#### Apply 14_disable_ui_password.patch
> Пароль стал encoded, удалил его из выдачи на UI. В реальном приложении для управления паролем необходим отдельный UI интерфейс с подтверждением старого пароля. 
